...
int package(const char *pkg_name, const char *dir_name)
{
	...
	name_p = cavan_path_copy(tmp_path, sizeof(tmp_path), dir_name, true);
...
int unpack(const char *pkg_name, const char *dir_name)
{
	...
	cavan_path_cat(tmp_path, sizeof(tmp_path), dir_name, HEADER_BIN_NAME, false);
	...
	while (pkg_info.image_count) {
		...
		cavan_path_cat(tmp_path, sizeof(tmp_path), dir_name, img_info.filename, false);
...
ssize_t swan_write_version(const char *system_mnt_point, u32 version)
{
	...
	cavan_path_cat(version_file, sizeof(version_file), system_mnt_point, SWAN_VERSION_PATH, false);
