...
int rle_to(const char *file_in, const char *file_out)
{
	...
	if (fd_in < 0) {
		pr_err_info("open input file \"%s\" failed", file_in);
		return -1;
	}

	fd_out = file_open_wo(file_out);
	if (fd_out < 0) {
		pr_err_info("open output file \"%s\" failed", file_out);
		ret = -1;
		goto out_close_in;
	}
