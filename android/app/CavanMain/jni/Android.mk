...
CAVAN_NATIVE_NAME := com_cavan_cavanmain_CavanNative
CAVAN_NATIVE_JAVA := $(LOCAL_PATH)/$(CAVAN_PROJECT_PATH)/src/$(subst _,/,$(CAVAN_NATIVE_NAME)).java
CAVAN_NATIVE_CPP := $(CAVAN_PROJECT_PATH)/jni/$(CAVAN_NATIVE_NAME).cpp
...
LOCAL_C_INCLUDES := $(CAVAN_C_INCLUDES)
LOCAL_SRC_FILES := $(CAVAN_PROJECT_PATH)/jni/CavanMain.cpp $(CAVAN_NATIVE_CPP) $(CAVAN_LIB_SRC_FILES)
...
$(LOCAL_PATH)/$(CAVAN_NATIVE_CPP): $(addprefix $(LOCAL_PATH)/,$(CAVAN_APP_SRC_FILES)) $(CAVAN_NATIVE_JAVA) | $(CAVAN_OUT_PATH)
	...
	$(hide) { \
		...
		do \
			...
			echo; \
			echo "extern \"C\" int do_cavan_$${app}(int argc, char *argv[]);"; \
			echo; \
			echo "JNIEXPORT jint Java_$(CAVAN_NATIVE_NAME)_native$${native_app}(JNIEnv *env, jobjectArray args)"; \
			echo "{"; \
...
$(CAVAN_NATIVE_JAVA): $(addprefix $(LOCAL_PATH)/,$(CAVAN_APP_SRC_FILES)) | $(CAVAN_OUT_PATH)
	...
	$(hide) { \
		...
		echo "\tstatic {"; \
		echo "\t\tSystem.loadLibrary(\"cavan\");"; \
		echo "\t}"; \
		echo; \
		for app in $(CAVAN_APP_LIST); \
		do \
			native_app=$$(echo $${app} | sed 's/\(^\|_\)\([a-z]\)/\u\2/g'); \
			echo "\tpublic static native int native$${native_app}(String[] args);"; \
		done; \
		echo "}"; \
	} > $@
