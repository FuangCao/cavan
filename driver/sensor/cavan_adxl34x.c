...
static int adxl34x_sensor_chip_readid(struct cavan_input_chip *chip)
{
	int ret;
	u8 id;
...
static int adxl34x_sensor_chip_set_active(struct cavan_input_chip *chip, bool enable)
{
	int ret;
	u8 value;
...
static int adxl34x_sensor_chip_event_handler(struct cavan_input_chip *chip)
{
	int ret;
	u8 irq_state;
	...
	cavan_input_chip_report_events(chip, &chip->isr_list);

	return 0;
}

static int adxl34x_acceleration_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	u8 fifo_state;
...
static int adxl34x_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
...
static void adxl34x_input_chip_remove(struct cavan_input_chip *chip)
{
...
static int __init cavan_adxl34x_init(void)
{
	pr_pos_info();
...
static void __exit cavan_adxl34x_exit(void)
{
	pr_pos_info();
...
module_init(cavan_adxl34x_init);
module_exit(cavan_adxl34x_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Cavan ADXL345/346 Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
