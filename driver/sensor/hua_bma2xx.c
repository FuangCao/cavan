...
static int bma2xx_sensor_chip_readid(struct hua_input_chip *chip)
{
	int ret;
	u8 value;
...
static int bma2xx_acceleration_event_handler(struct hua_input_chip *chip, struct hua_input_device *dev)
{
	int ret;
	...
	hua_sensor_report_vector(dev->input, package.x >> 6, package.y >> 6, package.z >> 6);

	return 0;
}

static int bma2xx_input_chip_probe(struct hua_input_chip *chip)
{
	int ret;
...
static void bma2xx_input_chip_remove(struct hua_input_chip *chip)
{
...
static struct hua_input_init_data bma2xx_init_data[] =
{
	{REG_BW_SEL, 0x0C},
	{REG_RANGE_SEL, 0x00},
#if 0
...
static int __init hua_bma2xx_init(void)
{
	pr_pos_info();
...
static void __exit hua_bma2xx_exit(void)
{
	pr_pos_info();
...
module_init(hua_bma2xx_init);
module_exit(hua_bma2xx_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Huamobile bma2xx Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
