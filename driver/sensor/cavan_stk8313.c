...
static int stk8313_sensor_chip_readid(struct cavan_input_chip *chip)
{
	int ret;
	u8 value;
...
static int stk8313_sensor_chip_read_reg24(struct cavan_input_chip *chip, u8 *value, int retry)
{
	int ret;
	u8 buff[2];
...
static int stk8313_sensor_chip_set_vd(struct cavan_input_chip *chip, int retry)
{
	int ret;
	u8 reg24, value;
...
static int stk8313_sensor_chip_set_active(struct cavan_input_chip *chip, bool enable)
{
	int ret;
	u8 value;
...
static int stk8313_acceleration_set_delay(struct cavan_input_device *dev, unsigned int delay)
{
	int ret;
	u8 value;
...
static int stk8313_acceleration_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	short x, y, z;
...
static int stk8313_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
...
static void stk8313_input_chip_remove(struct cavan_input_chip *chip)
{
...
static int __init cavan_stk8313_init(void)
{
	pr_pos_info();
...
static void __exit cavan_stk8313_exit(void)
{
	pr_pos_info();
...
module_init(cavan_stk8313_init);
module_exit(cavan_stk8313_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Cavan stk8313 Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
