...
static int mma845x_sensor_chip_readid(struct cavan_input_chip *chip)
{
	int ret;
	u8 id;
...
static int mma845x_sensor_chip_set_active(struct cavan_input_chip *chip, bool enable)
{
	int ret;
	u8 value;
...
static int mma845x_acceleration_set_enable(struct cavan_input_device *dev, bool enable)
{
	int ret;
	u8 value;
...
static int mma845x_acceleration_set_delay(struct cavan_input_device *dev, unsigned int delay)
{
	int ret;
	u8 value;
...
static int mma845x_acceleration_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	short x, y, z;
...
static int mma845x_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
...
static void mma845x_input_chip_remove(struct cavan_input_chip *chip)
{
...
static int __init cavan_mma845x_init(void)
{
	pr_pos_info();
...
static void __exit cavan_mma845x_exit(void)
{
	pr_pos_info();
...
module_init(cavan_mma845x_init);
module_exit(cavan_mma845x_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Cavan MMA845X Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
