...
static int lis3dh_sensor_chip_readid(struct cavan_input_chip *chip)
{
	int ret;
	u8 id;
...
static int lis3dh_sensor_chip_set_active(struct cavan_input_chip *chip, bool enable)
{
	int ret;
	u8 value;
...
static int lis3dh_acceleration_set_delay(struct cavan_input_device *dev, unsigned int delay)
{
...
static int lis3dh_acceleration_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
...
static int lis3de_acceleration_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
...
static int lis3dh_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
...
static void lis3dh_input_chip_remove(struct cavan_input_chip *chip)
{
...
static int __init cavan_lis3dh_init(void)
{
	pr_pos_info();
...
static void __exit cavan_lis3dh_exit(void)
{
	pr_pos_info();
...
module_init(cavan_lis3dh_init);
module_exit(cavan_lis3dh_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Cavan LIS3DH Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
