...
static inline int apds99xx_read_register(struct cavan_input_chip *chip, u8 addr, u8 *value)
{
	return cavan_input_read_register_i2c_smbus(chip, APDS99XX_CMD_BYTE | addr, value);
}

static inline int apds99xx_write_register(struct cavan_input_chip *chip, u8 addr, u8 value)
{
	return cavan_input_write_register_i2c_smbus(chip, APDS99XX_CMD_BYTE | addr, value);
}

static inline int apds99xx_read_register16(struct cavan_input_chip *chip, u8 addr, u16 *value)
{
	return cavan_input_read_register16_i2c_smbus(chip, APDS99XX_CMD_WORD | addr, value);
}

static inline int apds99xx_write_register16(struct cavan_input_chip *chip, u8 addr, u16 value)
{
	return cavan_input_write_register16_i2c_smbus(chip, APDS99XX_CMD_WORD | addr, value);
}

static inline int apds99xx_send_command(struct cavan_input_chip *chip, u8 command)
{
...
static int apds99xx_sensor_chip_readid(struct cavan_input_chip *chip)
{
	int ret;
	u8 value;
	const char *name;
...
static int apds99xx_sensor_chip_set_active(struct cavan_input_chip *chip, bool enable)
{
	int ret;
...
static int apds99xx_proximity_set_enable(struct cavan_input_device *dev, bool enable)
{
	int ret;
	u8 value;
...
static int apds99xx_proximity_set_delay(struct cavan_input_device *dev, unsigned int delay)
{
	pr_pos_info();
...
static ssize_t apds99xx_proximity_calibration(struct cavan_input_device *dev, char *buff, size_t size, bool store)
{
	int ret;
	s8 offset;
	struct cavan_input_chip *chip = dev->chip;
...
static int apds99xx_proximity_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	u16 value;
#if APDS99XX_SUPPORT_IRQ
	...
	cavan_sensor_report_value(dev->input, (APDS99XX_PROXI_MAX_VALUE - value) >> APDS99XX_PROXI_STEP);
...
static void apds99xx_update_alpc(struct cavan_apds99xx_device *apds99xx)
{
...
static int apds99xx_set_again(struct cavan_apds99xx_device *apds99xx, struct cavan_input_chip *chip, int again)
{
	int ret;
...
static int apds99xx_set_areduce(struct cavan_apds99xx_device *apds99xx, struct cavan_input_chip *chip, bool reduce)
{
	int ret;
...
static int apds99xx_light_set_enable(struct cavan_input_device *dev, bool enable)
{
	int ret;
	u8 value;
	struct cavan_input_chip *chip = dev->chip;
...
static int apds99xx_light_set_delay(struct cavan_input_device *dev, unsigned int delay)
{
	int ret;
	u16 ares;
	struct cavan_apds99xx_device *apds99xx;
...
static void apds99xx_light_report_value(struct cavan_input_device *dev, struct cavan_apds99xx_device *apds99xx, u16 ch0data, u16 ch1data)
{
	...
	cavan_sensor_report_value(dev->input, lux);
}

#if APDS99XX_SUPPORT_IRQ
static void apds99xx_set_ait(struct cavan_input_chip *chip, u32 data)
{
	u32 ailt, aiht;
...
static int apds99xx_light_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	u16 ch0data, ch1data;
	struct cavan_apds99xx_device *apds99xx = cavan_input_chip_get_dev_data(chip);
...
#if APDS99XX_SUPPORT_IRQ
static int apds99xx_chip_event_handler(struct cavan_input_chip *chip)
{
	int ret;
...
static int apds99xx_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
	...
	cavan_input_chip_set_dev_data(chip, apds99xx);
...
out_cavan_input_device_unregister_proxi:
	cavan_input_device_unregister(chip, &apds99xx->proxi.dev);
out_kfree_sensor:
	kfree(sensor);
	return ret;
}

static void apds99xx_input_chip_remove(struct cavan_input_chip *chip)
{
	struct cavan_apds99xx_device *apds99xx = cavan_input_chip_get_dev_data(chip);

	pr_pos_info();

	cavan_input_device_unregister(chip, &apds99xx->light.dev);
	cavan_input_device_unregister(chip, &apds99xx->proxi.dev);
	kfree(apds99xx);
}
...
static int __init cavan_apds99xx_init(void)
{
	pr_pos_info();
...
static void __exit cavan_apds99xx_exit(void)
{
	pr_pos_info();
...
module_init(cavan_apds99xx_init);
module_exit(cavan_apds99xx_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Cavan apds99xx Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
