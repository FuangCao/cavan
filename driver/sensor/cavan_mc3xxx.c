...
static int mc3xxx_sensor_chip_readid(struct cavan_input_chip *chip)
{
	int i;
	int ret;
	u8 value;
...
static int mc3xxx_sensor_chip_set_active_base(struct cavan_input_chip *chip, bool enable)
{
	return chip->write_register(chip, REG_MODE_FEATURE, enable ? 0x41 : 0x43);
}

static int mc3xxx_sensor_chip_set_active(struct cavan_input_chip *chip, bool enable)
{
	int ret;
	u8 value;
...
static int mc3xxx_acceleration_set_delay(struct cavan_input_device *dev, unsigned int delay)
{
	int ret;
	u8 value;
...
static int mc3xxx_acceleration_event_handler_high(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
...
static int mc3xxx_acceleration_event_handler_low(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
...
static int mc3xxx_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
...
static void mc3xxx_input_chip_remove(struct cavan_input_chip *chip)
{
...
static int __init cavan_mc3xxx_init(void)
{
	pr_pos_info();
...
static void __exit cavan_mc3xxx_exit(void)
{
	pr_pos_info();
...
module_init(cavan_mc3xxx_init);
module_exit(cavan_mc3xxx_exit);

MODULE_AUTHOR("Fuang.Cao <cavan.cfa@gmail.com>");
MODULE_DESCRIPTION("Cavan mc3xxx Three-Axis Digital Accelerometer Driver");
MODULE_LICENSE("GPL");
