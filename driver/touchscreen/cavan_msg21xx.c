...
#define CAVAN_SUPPORT_PROXIMITY		0   /* 1:open the tp proximity  0:close the tp proximity,closed by chenzili at 2014-5-28 for zc2501_tw*/
...
#define MSG21XX_XAXIS_MAX			(CONFIG_CAVAN_LCD_WIDTH)
#define MSG21XX_YAXIS_MIN			0
#define MSG21XX_YAXIS_MAX			(CONFIG_CAVAN_LCD_HEIGHT)
...
static ssize_t msg21xx_read_data(struct cavan_input_chip *chip, u8 addr, void *buff, size_t size)
{
	return cavan_input_master_recv_from_i2c(cavan_input_chip_get_bus_data(chip), addr, buff, size);
}

static ssize_t msg21xx_write_data(struct cavan_input_chip *chip, u8 addr, const void *buff, size_t size)
{
	return cavan_input_master_send_to_i2c(cavan_input_chip_get_bus_data(chip), addr, buff, size);
}

static ssize_t msg21xx_read_data_package(struct cavan_input_chip *chip, struct msg21xx_data_package *package)
{
	return chip->master_recv(chip, package, sizeof(*package));
}

static int msg21xx_ts_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
...
#if CAVAN_SUPPORT_PROXIMITY
static int msg21xx_proximity_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	struct cavan_msg21xx_chip *msg21xx = container_of(chip, struct cavan_msg21xx_chip, chip);

	cavan_sensor_report_value(dev->input, msg21xx->distance);

	return 0;
}

static int msg21xx_proximity_set_enable(struct cavan_input_device *dev, bool enable)
{
	struct cavan_input_chip *chip = dev->chip;
	u8 buff[] = {0x52, 0x00, 0x4a, enable ? 0xa0 : 0xa1};

	cavan_input_print_memory(buff, sizeof(buff));

	return chip->master_send(chip, buff, sizeof(buff));
}
#endif

static void msg21xx_chip_reset(struct cavan_input_chip *chip)
{
...
static int msg21xx_read_firmware_id(struct cavan_input_chip *chip, char *buff, size_t size)
{
	int ret;
	u16 version[2];
...
static int msg21xx_readid(struct cavan_input_chip *chip)
{
	int ret;
	char buff[32];
...
static int msg21xx_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;

	struct cavan_msg21xx_device *dev;
	...
	base_dev->type = CAVAN_INPUT_DEVICE_TYPE_PROXIMITY;
	base_dev->use_irq = true;
...
out_cavan_input_device_unregister_prox:
#if CAVAN_SUPPORT_PROXIMITY
...
static int msg21xx_chip_event_handler(struct cavan_input_chip *chip)
{
	int ret;
...
static void msg21xx_input_chip_remove(struct cavan_input_chip *chip)
{
	struct cavan_msg21xx_device *dev = cavan_input_chip_get_dev_data(chip);
...
int msg21xx_chip_firmware_upgrade(struct cavan_input_chip *chip, struct cavan_firmware *fw)
{
...
static int msg21xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct cavan_input_chip *chip;
	struct cavan_msg21xx_chip *msg21xx;
...
#if CAVAN_SUPPORT_PROXIMITY
	chip->devmask |= 1 << CAVAN_INPUT_DEVICE_TYPE_PROXIMITY;
#endif
...
{
	struct cavan_msg21xx_chip *msg21xx = i2c_get_clientdata(client);
	struct cavan_input_chip *chip = &msg21xx->chip;
...
MODULE_DESCRIPTION("Cavan MSG21XX TouchScreen Driver");
MODULE_LICENSE("GPL");
