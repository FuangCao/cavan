...
#if defined CONFIG_CAVAN_D58 || defined CONFIG_CAVAN_H2 || defined CONFIG_CAVAN_H4
...
#define FT5216_XAXIS_MAX			(CONFIG_CAVAN_LCD_WIDTH + 2)
#define FT5216_YAXIS_MIN			0
#define FT5216_YAXIS_MAX			CONFIG_CAVAN_LCD_HEIGHT
...
static inline ssize_t ft5216_read_data_package(struct cavan_input_chip *chip, struct ft5216_data_package *package)
{
	return chip->read_data(chip, 0, package, sizeof(*package));
}

static int ft5216_send_command(struct cavan_input_chip *chip, u8 *command, size_t wrlen, size_t rdlen)
{
	int ret;
...
static int ft5216_send_command_simple(struct cavan_input_chip *chip, char command)
{
...
static int ft5216_change_power_mode(struct cavan_input_chip *chip, u8 mode, int retry)
{
...
static int ft5216_ts_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	int count;
...
#if SUPPORT_PROXIMITY_SENSOR
static int ft5216_proximity_event_handler(struct cavan_input_chip *chip, struct cavan_input_device *dev)
{
	int ret;
	u8 value;
	...
	case 0xE0:
		cavan_sensor_report_value(dev->input, 1);
		break;
	}
...
static int ft5216_proximity_set_enable(struct cavan_input_device *dev, bool enable)
{
...
static int ft5216_set_active(struct cavan_input_chip *chip, bool enable)
{
	u8 mode;
	int retry;
...
static int ft5216_readid(struct cavan_input_chip *chip)
{
	int ret;
	char buff[94];
	const u8 *p, *p_end;
...
static int ft5216_read_firmware_id(struct cavan_input_chip *chip)
{
	int ret;
	u8 command = 0xa6;
...
static int ft5216_read_vendor_id(struct cavan_input_chip *chip)
{
	int ret;
	u8 command = 0xa8;
...
static int ft5216_firmware_write_block(struct cavan_input_chip *chip, struct ft5216_firmware_block *block, u16 addr, u16 size)
{
...
static int ft5216_upgrade_prepare(struct cavan_input_chip *chip)
{
	int ret;
...
static int ft5216_enter_upgrade_mode(struct cavan_input_chip *chip, int retry)
{
...
static int ft5216_check_hardware_id(struct cavan_input_chip *chip)
{
	int ret;
...
static int ft5216_firmware_erase_app(struct cavan_input_chip *chip)
{
	pr_pos_info();
...
static int ft5216_firmware_read_ecc(struct cavan_input_chip *chip, unsigned char *ecc)
{
	*ecc = 0xcc;
...
static int ft5216_firmware_reset(struct cavan_input_chip *chip)
{
	int ret;
...
static int ft5216_firmware_upgrade(struct cavan_input_chip *chip, struct cavan_firmware *fw)
{
	int ret;
	ssize_t rdlen;
	u16 addr;
...
static ssize_t ft5216_ts_calibration(struct cavan_input_device *dev, char *buff, size_t size, bool store)
{
	int ret;
	u8 value;
	int i;
...
static ssize_t ft5216_chip_read_firmware_id(struct cavan_input_chip *chip, char *buff, size_t size)
{
	int ret;
	u8 vendor, version;
...
static int ft5216_input_chip_probe(struct cavan_input_chip *chip)
{
	int ret;
	struct cavan_ft5216_driver_data *data;
	struct cavan_ts_device *ts;
#if SUPPORT_PROXIMITY_SENSOR
	struct cavan_sensor_device *prox;
#endif
	...
	cavan_input_chip_set_dev_data(chip, data);
	...
	dev = &ts->dev;
	dev->type = CAVAN_INPUT_DEVICE_TYPE_TOUCHSCREEN;
	dev->use_irq = true;
...
out_cavan_input_device_unregister_proxy:
#if SUPPORT_PROXIMITY_SENSOR
	cavan_input_device_unregister(chip, &prox->dev);
out_cavan_input_device_unregister_ts:
#endif
	cavan_input_device_unregister(chip, &ts->dev);
out_kfree_data:
	kfree(data);
	return 0;
}

static void ft5216_input_chip_remove(struct cavan_input_chip *chip)
{
...
#endif
	cavan_input_device_unregister(chip, &data->ts.dev);
	kfree(data);
}
...
#else
	chip->devmask = 1 << CAVAN_INPUT_DEVICE_TYPE_TOUCHSCREEN;
#endif
...
MODULE_DESCRIPTION("Cavan FocalTech FT5216 TouchScreen Driver");
MODULE_LICENSE("GPL");
