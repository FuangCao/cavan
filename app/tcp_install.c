...
#define FILE_CREATE_DATE "2016-05-25 18:54:02"
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "vVhHi:I:p:P:AalLu:U:fo", long_option, &option_index)) != EOF) {
		switch (c) {
		case 'v':
		case 'V':
	...
	while (optind < argc) {
		int ret = tcp_dd_install(&url, argv[optind++]);
		if (ret < 0) {
			pr_red_info("tcp_dd_install: %d", ret);
			return ret;
		}
	}
