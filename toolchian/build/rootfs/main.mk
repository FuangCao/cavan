...
$(SB2_INIT_MARK):
	$(Q)cd $(ROOTFS_PATH) && mkdir bin sbin root home tmp proc sys dev etc usr/bin usr/sbin -pv && ln -vsf bash bin/sh -vsf
	$(Q)cp $(SYSROOT_PATH)/lib $(ROOTFS_PATH) -av
	$(Q)cp $(SYSROOT_PATH)/usr/lib $(SYSROOT_PATH)/usr/$(CAVAN_TARGET_PLAT)/lib $(ROOTFS_PATH)/usr -av
	$(Q)cp $(SYSROOT_PATH)/usr/include $(SYSROOT_PATH)/usr/$(CAVAN_TARGET_PLAT)/include $(ROOTFS_PATH)/usr -av
