	...
	public byte[] parse(BufferedReader reader) {
		try {
			...
			while (true) {
				...
				byte[] bytes = CavanUtils.parseHexText(line.getBytes(), 1, line.length() - 1);
				int length = bytes[0];
				...
				case 0x00:
					if (offset + length > binData.length) {
						...
						CavanUtils.ArrayCopy(binData, datas, sizeReal);
						binData = datas;
					}
				...
				case 0x01:
					return CavanUtils.ArrayCopy(binData, 0, sizeReal);
